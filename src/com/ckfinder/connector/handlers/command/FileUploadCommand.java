...
public class FileUploadCommand extends Command implements IPostCommand {
	...
	private boolean fileUpload(final HttpServletRequest request) {
		try {
			...
			String iswater = getParameter(request, "iswater");
			if(StringUtils.isBlank(iswater)){
				iswater="0";
			}
			//System.out.println("id:"+iswater);
				...
				if (!item.isFormField()) {
					...
					try {
						if (validateUploadItem(item, path)) {
							return saveTemporaryFile(path, item,iswater);
						}
				 	} finally {
						item.delete();
					}
	 ...
	 */
	private boolean saveTemporaryFile(final String path, final FileItem item,String iswater)
			throws Exception {
		//上传后的文件名称
		...
		if (!ImageUtils.isImage(file)) {
			...
			System.out.println(path+":"+this.newFileName);
			//图片
			ImageUtils.createTmpThumb(item.getInputStream(), file,
					getFileItemName(item), this.configuration);
			System.out.println(args.getCurrentFolder());
			System.out.println(EventTypes.AfterFileUpload);
			System.out.println(configuration.getThumbsDir());
//			if (configuration.getEvents() != null) {
//				configuration.getEvents().run(EventTypes.AfterFileUpload,
//						args, configuration);
//			}
...
//			 */
//			if(!iswater.equals("0")){
//				createWater(paths,path,this.newFileName,iswater);
//			}
			return true;
	 ...
	 * filepath,图片路径
	 */
	public static void createWater(String path,String filepath,String filename,String type){
			String value=""; //水印图片所在路径
			if(type.trim().equals("1")){
				value=PropertiesUtil.getConfig(path,"bwatermark");
			}else if(type.trim().equals("2")){
				value=PropertiesUtil.getConfig(path,"swatermark");
			}
			boolean flags=PressImage.pressImage(filepath+filename,value);
	}
	/**
...
//			return (m.find()) ? m.group(0) : "";
		 		...
		 		nowtime=nowtime.replace(":","");
		 		Date date=new Date();     
		 		nowtime=nowtime+date.getTime();	
			  String origin = item.getName();  
			//  System.out.println("处理后的名字："+nowtime + "." + FileUtils.getFileExtension(origin));
		      return  nowtime + "." + FileUtils.getFileExtension(origin);   
	}
