...
import dty.expand.util.ContentPageUtil;
...
public class InfoService extends BaseService{
	...
	public InfoService(){
		...
		init("channelService","siteService","innerlinkService");
	}
	/**
	 * 生成SiteMap.xml文件
	 */
	public void creatSiteMapXML(String path){
			System.out.println("start creating Sitemap.xml...");
			String sites="0b82baf0-4fc5-4784-ad9e-e8a046213ea8";
			List<Channel> chnllist=null;
			List<Info> infolist=null;
			chnllist=channelService.findBySite(sites,"1","");
			//栏目id集合
			List<String> chanid=new ArrayList<String>();
			for(int i=0;i<chnllist.size();i++){
				chanid.add(chnllist.get(i).getId());
			}
			Info xmlinfo=new Info();
			xmlinfo.setSite(sites);
			xmlinfo.setChannelids(chanid);
			infolist=findAll(xmlinfo," addtime desc");
			Document document=XMLUtil.createInfoXML(chnllist,infolist);
			System.out.println(chnllist.size()+":"+infolist.size());
			
			XMLUtil.writeFile(document,path);
			//XMLUtil.writeFile(document,"d://test//Sitemap.xml");
	}
	/**
	 * 生成信息页
	 * @param id
	...
	public void html(String id,ServletContext context,String contextPath,HttpServletRequest request,String operuser) throws IOException, TemplateException{
		...
		if (info!=null) {
			...
			if (site!=null && site.getIndextemplet()!=null && site.getIndextemplet().trim().length()>0) {
				...
				/**
				 * update by limengyu at 20131024
				 * 信息页静态化时，将文档的正文传递过去
				 */
				String infocontent=info.getContent();
				String content=ContentPageUtil.pageBreak(infocontent);
				//获取内链列表,处理内链
				List<Innerlink> link_list=innerlinkService.findAllInnerLink();
				//正文-内链
				content=ContentPageUtil.convertContent(link_list,content);
				data.put("currInfoContent",content);
				
				//生成目录
				String rootFolder=request.getRealPath("/")+"details/"+(info.getAddtime().getYear()+1900)+"/";
				File folder=new File(rootFolder);
				if (!folder.exists()) {
					folder.mkdirs();
				}
				FreeMarkerUtil.createHTML(context, data,"templet/"+site.getIndextemplet().trim()+"/"+templet, 
						rootFolder+info.getHtmlFileName()+".shtml");
				if(info.getIscheck().equalsIgnoreCase("0")){
					File file=new File(rootFolder+info.getHtmlFileName()+".shtml");
					if (file.exists()) {
						file.delete();
					}
				}
				//显示全文的页面
				String contentAll=ContentPageUtil.getContentByPageAll(infocontent);
				//显示全文的内链转换
				contentAll=ContentPageUtil.convertContent(link_list,contentAll);
				data.put("currInfoContent",contentAll);
				FreeMarkerUtil.createHTML(context,data,"templet/"+site.getIndextemplet().trim()+"/"+templet, 
						rootFolder+info.getHtmlFileName()+"_all.shtml");
				if(info.getIscheck().equalsIgnoreCase("0")){
					File file=new File(rootFolder+info.getHtmlFileName()+"_all.shtml");
					if (file.exists()) {
						file.delete();
					}
				}
				/**
				 * update by limengyu at 20131025
				 */
				String path=rootFolder+info.getHtmlFileName().toString();
				data.put("paths",path);
				int count=ContentPageUtil.getPageCount(infocontent);//获取到总页数
				//显示各个分页的页面
				for(int i=1;i<=count;i++){
					String contentPageData=ContentPageUtil.getContentByPageId(infocontent,i);
					//显示各个分页的内链转换
					contentPageData=ContentPageUtil.convertContent(link_list,contentPageData);
					data.put("currInfoContent",contentPageData);
					FreeMarkerUtil.createHTML(context, data,"templet/"+site.getIndextemplet().trim()+"/"+templet, 
							path.toString()+"_"+i+".shtml");
					//System.out.println(path+"=============="+i+"\n");
					if(info.getIscheck().equalsIgnoreCase("0")){
						File file=new File(path.toString()+"_"+i+".shtml");
						if (file.exists()) {
							file.delete();
						}
					}
	...
	public void delhtml(String id,HttpServletRequest request) throws IOException, TemplateException{
		...
		if (info!=null) {
			...
			if (site!=null ) {
				//删除静态文件
				String htmlfile=request.getRealPath("/")+"/details/"+(info.getAddtime().getYear()+1900)+"/"+info.getHtmlFileName()+".shtml";
	...
	public List<Info> find(Info info,String order,int currPage,int pageSize){
		...
		if(StringUtils.isNotBlank(info.getAddtimeBetween())){
			Calendar cal=Calendar.getInstance();
			if(info.getAddtimeBetween().equalsIgnoreCase("day")){
		        cal.add(Calendar.DATE,-1);
		        Date time=cal.getTime();
				criteria.andAddtimeGreaterThanOrEqualTo(time);
			}
			if(info.getAddtimeBetween().equalsIgnoreCase("week")){
		        cal.add(Calendar.DATE,-7);
		        Date time=cal.getTime();
				criteria.andAddtimeGreaterThanOrEqualTo(time);
			}
			if(info.getAddtimeBetween().equalsIgnoreCase("month")){
		        cal.add(Calendar.DATE,-30);
		        Date time=cal.getTime();
				criteria.andAddtimeGreaterThanOrEqualTo(time);
			}
			if(info.getAddtimeBetween().equalsIgnoreCase("year")){
	 ...
	 * @return
	 */
	public List<Info> findInfoByZT(Info info,String order,int start,int end){
		InfoExample example=new InfoExample();
		Criteria criteria=example.createCriteria();
		proSearchParam(info, criteria);
		if (order!=null && order.trim().length()>0) {
		...
		return infoMapper.findInfoByZT(example);
	}
	/**
	 * 第几期
	 * 获取到当前最大的期数
	 * @return	获取到当前最大的期数+1
	 */
	public int getMaxNper(){
		int n=0;
		n=infoMapper.selectMaxNper();
		n=n+1;
		return n;
	}
	/**
	...
	public void proSearchParam(Info info,Criteria criteria){
		if (info!=null ) {
			...
			if (info.getSearchKey()!=null && info.getSearchKey().trim().length()>0) {
				//update by limengyu @20131109
				//判断检索的方式，0为全文检索，1为标题检索
				String way="0";
				way=info.getWay();
			...
			//limengyu add by 20131111
			if(info.getIshot()!=null && info.getIshot().trim().length()>0){
				criteria.andIshotEqualTo(info.getIshot());
			}
			...
			/**
			 * update by limengyu @20131112
			 * 是否读取有效并且是导航的栏目，1为是
			 */
			if(info.getScope()!=null && info.getScope().length()>0){
				if(info.getScope().equals("1")){
					criteria.andSql(" (channel in (select id from freecms_channel where site='"+info.getSite()+"' and state='1' and navigation='1'))");
				}
			}
			 ...
			 * 1是否在首页显示、是否在列表页显示、推荐
			 */
			if (info.getIsshowindex()!=null && info.getIsshowindex().length()>0) {
				criteria.andIsshowindexEqualTo(info.getIsshowindex());
			}
			if(info.getIsshowlist()!=null && info.getIsshowlist().length()>0){
				criteria.andIsshowlistEqualTo(info.getIsshowlist());
			}
			if(info.getIsrecommend()!=null && info.getIsrecommend().length()>0){
				criteria.andIsrecommendEqualTo(info.getIsrecommend());	
			}
			//where id！=info.getIsnotinid()
			if(info.getIsnotinid()!=null && info.getIsnotinid().length()>0){
				criteria.andInfoIdNotEqualTo(info.getIsnotinid());
			}
			//设置是否可以发布
			if(info.getIscheck()!=null && info.getIscheck().length()>0){
				criteria.andIscheckEqualTo(info.getIscheck());
			}
			
			//设置来源
			if(info.getSource()!=null && info.getSource().length()>0){
				criteria.andSourceEqualTo(info.getSource());
			}
			
			//是否在新闻中心栏目显示 1为显示，0为不显示
			if(info.getIsshownewscenter()!=null && info.getIsshownewscenter().length()>0){
				criteria.andIsshownewscenterEqualTo(info.getIsshownewscenter());
			}
			
			//是否在新闻中心栏目置顶，1为置顶，0为不置顶
			if(info.getIstopnewscenter()!=null && info.getIstopnewscenter().length()>0){
				criteria.andIstopnewscenterEqualTo(info.getIstopnewscenter());
			}
			
			//是否在新闻中心栏目推荐，1为推荐，0为不推荐
			if(info.getIsrecommendnewscenter()!=null && info.getIsrecommendnewscenter().length()>0){
				criteria.andIsrecommendnewscenterEqualTo(info.getIsrecommendnewscenter());
			}
		}
	}
	/**
	...
	public List<Info> findByTitle(String title){
		...
		return infoMapper.findByTitle(title);	
	}
	...
	public void delQuoteById(String id){
		infoMapper.delQuoteById(id);
		DBCommit();
	}
	/**
	 * 工作量统计 
     ...
     * @return
     */
	public int sourceCount(Info info){
		InfoExample example=new InfoExample();
		Criteria criteria=example.createCriteria();
		proSearchParam(info, criteria);
		return infoMapper.sourceCount(example);
