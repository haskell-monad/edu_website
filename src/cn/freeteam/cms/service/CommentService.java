...
public class CommentService extends BaseService{
	...
	public String add(Comment Comment){
		...
		if(Comment.getIsRandomAddTime()==1){
			SimpleDateFormat df = new SimpleDateFormat("yyyyMMdd");
			String yyyyMMdd = df.format(new Date());
			Random random = new Random();
			String addtimeStr = "";
			int HHStart = Comment.getInfoAddTimeHours();
			int HHEnd = 23;
			HHStart=HHStart==HHEnd?HHEnd:HHStart+1;
			String HH = "" + DateTimeUtil.getRandomBetween(HHStart, HHEnd);
			HH = HH.length()==1?"0"+HH:HH;
			String mm = "" + random.nextInt(6) + random.nextInt(10);
			String ss = "" + random.nextInt(6) + random.nextInt(10);
			addtimeStr = yyyyMMdd + HH + mm + ss;
			Comment.setAddtime(DateTimeUtil.StrToDate(addtimeStr, "yyyyMMddHHmmss"));
		}
