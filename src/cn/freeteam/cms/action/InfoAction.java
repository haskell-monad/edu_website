...
public class InfoAction extends BaseAction{
	...
	//update by limengyu 2013.10.02
	private String lmy_channelids;
	
	public String getLmy_channelids() {
		return lmy_channelids;
	}
	public void setLmy_channelids(String lmy_channelids) {
		this.lmy_channelids = lmy_channelids;
	}
	...
	public InfoAction(){
		init("siteService","channelService","infoService",
				"roleChannelService","userService","sourceService");
	}
	/**
	 * 信息列表
	 * @return
	...
	public String list(){
		...
		for(int i=0;i<infoList.size();i++){
		    if(infoList.get(i)!=null && infoList.get(i).getIsquote()==1 
		            && StringUtils.isNotEmpty(infoList.get(i).getQuoteinfoid())){
		       String fatherId = infoList.get(i).getQuoteinfoid();
		       infoList.get(i).setPageurl(infoService.findInfoById(fatherId).getPageurl());
//		       infoList.get(i).setHtmlIndexnum(infoService.findInfoById(fatherId).getHtmlIndexnum());
//		       infoList.get(i).setHtmlFileName(infoService.findInfoById(fatherId).getHtmlFileName());
		    }
		}
	...
	public String extract(){
		...
		Pager pager=new Pager(getHttpRequest());
		pager.appendParam("info.channel");
		pager.appendParam("info.htmlIndexnum");
	...
	public String yeditDo(){
		...
		if (info!=null) {
			...
			try {
				 ...
				 * 设置期数
				 */
				info.setNper(infoService.getMaxNper());
				...
				if (info.getId()!=null && info.getId().trim().length()>0) {
					...
					if (oldInfo!=null) {
						...
						oldInfo.setChannelname(info.getChannelname());
						...
						oldInfo.setEditor(info.getEditor());
						oldInfo.setClicknum(info.getClicknum());
						oldInfo.setKeywords(info.getKeywords());
						oldInfo.setRecommendreason(info.getRecommendreason());
						oldInfo.setSuitage(info.getSuitage());
						oldInfo.setDepth(info.getDepth());
						oldInfo.setIscheck(info.getIscheck());
						oldInfo.setEditor(info.getEditor());
						oldInfo.setFirstvalue(info.getFirstvalue());
						oldInfo.setIsshowindex(info.getIsshowindex());
						oldInfo.setIsshowlist(info.getIsshowlist());
						oldInfo.setIsrecommend(info.getIsrecommend());
						oldInfo.setIsshownewscenter(info.getIsshownewscenter()); //是否在新闻中心显示 1为显示
						oldInfo.setIstopnewscenter(info.getIstopnewscenter());
						oldInfo.setIsrecommendnewscenter(info.getIsrecommendnewscenter());
						/**
						 * update by lilmengyu @20131202
						 * 青年建筑师
						 * gzlm关注栏目
						 * bjyx编辑邮箱
						 * nper期数
						 */
						oldInfo.setGzlm(info.getGzlm());
						oldInfo.setBjyx(info.getBjyx());
						//oldInfo.setNper(info.getNper());
						oldInfo.setMdtime(new Date());
					...
					/**
					 * update by limengyu
					 * 获取到栏目id数组（在多个栏目中添加相同的一条数据）
					 */
					String oldchannelid=info.getChannel();
					String[] channelids_lmy=null;
					String channelid_lmy="";
					channelids_lmy= lmy_channelids.split(";");
					for(int i=0;i<channelids_lmy.length;i++){
						channelid_lmy=(String)channelids_lmy[i];
						if(channelid_lmy!=null && !channelid_lmy.trim().equals("") && channelid_lmy.length()>0){
							Channel yadd_channel=channelService.findById(channelid_lmy);
							info.setChannel(channelid_lmy);
							info.setChannelname(yadd_channel.getName());
							infoService.insert(info);
						}
					}
					info.setChannel(oldchannelid);
				...
				*/
				if (ismakehtml) {
					...
					if ("channel".equals(type)) {
						write("<script>alert('操作成功');location.href='info_ylist.do?info.channel="+info.getChannel()+"&pageFuncId="+pageFuncId+"';</script>", "GBK");
					}else {
						write("<script>alert('操作成功');location.href='info_yedit.do?pageFuncId="+pageFuncId+"';</script>", "GBK");
					}
				}
			} catch (Exception e) {
				DBProException(e);
			...
			try {
				...
				if (info.getId()!=null && info.getId().trim().length()>0) {
					...
					if (oldInfo!=null) {
						...
						oldInfo.setShorttitle(info.getShorttitle());
						oldInfo.setSite(info.getSite());
						//判断来源是否在数据库中存在，如果不存在就保存在数据库中
						boolean flag=sourceService.isexist(info.getEditor(),info.getSource());
						oldInfo.setSource(info.getSource());
						oldInfo.setTags(info.getTags());
						oldInfo.setTemplet(info.getTemplet());
						oldInfo.setTitle(info.getTitle());
						...
						oldInfo.setIsrecommend(info.getIsrecommend());
						//是否在新闻中心显示 1为显示0为不显示
						oldInfo.setIsshownewscenter(info.getIsshownewscenter());
						oldInfo.setIstopnewscenter(info.getIstopnewscenter());
						oldInfo.setIsrecommendnewscenter(info.getIsrecommendnewscenter());
						
						//设置修改人和修改时间
						oldInfo.setModifier(getLoginAdmin().getName());
						oldInfo.setMdtime(new Date());
						infoService.update(oldInfo);
						
						//根据栏目id获取当前文章所属栏目,然后更新栏目的最后修改时间
						Channel oldChannel = null;
						oldChannel = channelService.findById(oldInfo.getChannel());
						oldChannel.setMdtime(new Date());  //设置栏目最后修改时间
					...
					info.setClicknum(randoms(100,500));
					info.setAdduser(getLoginAdmin().getId());
					//设置修改人和修改时间
					info.setModifier(info.getEditor());
					info.setMdtime(new Date());
					...
					for(int i=0;i<channelids_lmy.length;i++){
						...
						if(channelid_lmy!=null && !channelid_lmy.trim().equals("") && channelid_lmy.length()>0){
							...
							info.setChannel(channelid_lmy);
							//根据栏目id获取栏目对象
							Channel add_channel=channelService.findById(channelid_lmy);
							if(add_channel!=null){
								//设置栏目名称
								info.setChannelname(add_channel.getName());
							}
							//保存文章
							infoService.insert(info);
						}
	 ...
	 * @return
	 */
	public String updateFirstValue(){
		if (info!=null) {
					...
					for (int i = 0; i < idArr.length; i++) {
						if (idArr[i].trim().length()>0) {
							...
							if (info!=null) {
								...
								sb.append(idArr[i]+";");
								//删除信息时删除引用信息
								infoService.delQuoteById(idArr[i]);
	...
	public String quote(){
		if (ids!=null && ids.trim().length()>0) {
			...
			if (idArr!=null && idArr.length>0 && tochannelid!=null ) {
				Channel toChannel=channelService.findById(tochannelid);
				...
				try {
					...
					for (int i = 0; i < idArr.length; i++) {
						if (idArr[i].trim().length()>0) {
							info=infoService.findInfoById(idArr[i]);
							if(info.getIsquote() == 1){
								idArr[i] = info.getQuoteinfoid();
								info = infoService.findInfoById(info.getQuoteinfoid());
							}
							oldChannel=channelService.findById(info.getChannel());
							if(tochannelid != oldChannel.getId() && !tochannelid.equals(oldChannel.getId()) ){
								if (info!=null) {
									Info newInfo = info;
									newInfo.setChannel(tochannelid);
									newInfo.setId("");
									newInfo.setTitle("");
									newInfo.setShorttitle("");
									newInfo.setSource("");
									newInfo.setAuthor("");
									newInfo.setDescription("");
									newInfo.setContent("");
									newInfo.setTags("");
									newInfo.setImg("");
									newInfo.setUrl("");
									newInfo.setAttchs("");
									newInfo.setId(UUID.randomUUID().toString());
									System.out.println("info.getPageurl():"+info.getPageurl());
//									newInfo.setPageurl(info.getPageurl());
									newInfo.setIsquote(1);
									newInfo.setQuoteinfoid(idArr[i]);
									
									infoService.insert(newInfo);
									logContent="引用信息("+oldChannel.getName()+" >> "+toChannel.getName()+" "+info.getTitle()+")成功!";
								}
								OperLogUtil.log(getLoginName(), logContent, getHttpRequest());
							
							}else{
								return showMessage("当前栏目下已存在该条数据", "", forwardSeconds);
							}
						}
					}
					//处理静态化
					...
					*/
					if (ismakehtml) {
						return "quoteMakehtml";
					}else {
						showMessage="引用信息成功!";
					}
				} catch (Exception e) {
					DBProException(e);
					logContent="引用信息("+oldChannel.getName()+" >> "+toChannel.getName()+" "+info.getTitle()+")失败:"+e.toString()+"!";
					OperLogUtil.log(getLoginName(), logContent, getHttpRequest());
					showMessage="引用信息失败:"+e.getMessage();
				}
				}
			}
		}
