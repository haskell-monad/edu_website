...
<img style="cursor:hand" onclick="window.open('<%=basePath %>index.shtml');" title="点击预览站点" src="../img/www.gif">
</s:if>
<s:if test="%{#session.manageSite == null}">
...
<ul class="container">
	...
	<s:iterator value="#session.funcs" id="bean">
	    <s:if test="%{#bean.parid==funcid}">
			<s:if test="%{name!='职位管理' && name!='简历管理' && name!='互动信件' && name!='在线申报' && name!='依申请公开'}">
			      <li class="menu">
			          	<ul>
						    <li class="button">
					            ...
					            <li class="dropdown">
					                <ul>
										...
										<s:if test="%{#sonbean.parid==#bean.id}">
					                    	<li><a <s:if test="%{name==\"直播后台管理\" || name==\"文库后台管理\"}">target="_blank"</s:if><s:else>target="right"</s:else><s:if test="%{url!=''}">href="<s:property value="url"/><s:if test="%{url.indexOf('?')>-1}">&pageFuncId=<s:property value="id"/></s:if><s:else>?pageFuncId=<s:property value="id"/></s:else>"</s:if> ><s:property value="name"/></a></li>
					                    </s:if>
					                    </s:iterator>
					                </ul>
								</li>
